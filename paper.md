...
|---------|-------------|---------|
| `QMToAIMWorkchain` | aimall.qmtoaim | Run a general computational chemistry software and link it to an AIMAll calculation |
| `GenerateWFXToAIMWorkchain` | aimall.wfxtoaim | Take non-standard AIMAll input files, and run AIMAll |
| `GaussianToAIMWorkChain` | aimall.g16toaim | Run a Gaussian calculation and automatically run an AIMAll calculation on its outputs |
| `SubstituentParameterWorkChain` | aimall.subparam | Compute substituent properties defined by the authors automatically |
